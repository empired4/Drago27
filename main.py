...
async def utk(course_id):
    ...
    def process_content_item(content_item, fi, subject_name, key, iv):
        try:
            ji = content_item.get("id")
            jt = content_item.get("title")
            jti = content_item["payload"]["tile_id"]
            ...
            if content_type == "video":
                        ...
                        for index in [3, 2, 1, 0]:
                            if len(qo) > index:
                                ...
                                if selected_vu:
                                    ...
                                    return f"VIDEO: ({subject_name}) | {jt} : {pu}\n"
                    else:
                        vu = cj.get("link", "")
                        if vu:
                ...
                j5 = post_request(meta_source_url, j4, key=key, iv=iv)
                test_data = j5.get("data", {})
                
                if test_data:
                    # Format test series as HTML
                    html_content = f"""
                    <div class="test-series">
                        <h3>{html.escape(jt)}</h3>
                        <p><strong>Subject:</strong> {html.escape(subject_name)}</p>
                        <p><strong>Test ID:</strong> {ji}</p>
                        <p><strong>Tile ID:</strong> {jti}</p>
                    </div>
                    """
                    return f"TEST_SERIES: {html_content}\n"
                ...
                j5 = post_request(meta_source_url, j4, key=key, iv=iv)
                pdf_data = j5.get("data", {})
                
                if pdf_data:
                    pdf_url = pdf_data.get("link", "")
                    if pdf_url:
                        pdf_url = pdf_url.split("?Expires=")[0]
                        return f"PDF: ({subject_name}) | {jt} : {pdf_url}\n"
        
        except Exception as e:
            print(f"⚠️ Content processing error: {content_item.get('title')} - {e}")
        return None
            ...
            for subj in dr4["data"]["list"]:
                ...
                for topic in dr5["data"]["list"]:
                    ...
                    if "data" in dr6 and "list" in dr6["data"]:
                        content_items = dr6["data"]["list"]
                        with ThreadPoolExecutor(max_workers=200) as executor:
                            futures = [
                                executor.submit(process_content_item, v, fi, sfn, key, iv)
                                for v in content_items
                            ]
                            for future in as_completed(futures):
...
async def get_course_id(client: Client, message: Message):
    try:
        ...
        await utk(course_id)  # 🔁 Your own function that extracts course and saves 'final.txt'
